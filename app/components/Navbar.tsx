...
import ChainbaseIcon from "./ChainbaseIcon";
...
const Dropdown: FC<DropdownProps> = ({ title, links }) => {
  ...
  return (
    ...
    >
      <button
        className={`w-20 flex items-center justify-center text-base whitespace-nowrap font-satoshi pb-4 px-3 text-purple-500  ${
          isOpen ? "!font-black" : ""
        } ${
          links.some((link) => isOptionActive(link.href))
            ? "!font-black !text-purple-500 !border-b-2 !border-black"
            : ""
        }`}
      >
        {title}
      ...
      {isOpen && (
        <div className="absolute left-0 border p-2 z-50 w-60 bg-purple-500 rounded-bl-sm rounded-br-sm">
...
const NavbarLink: FC<NavbarLinkProps> = ({ href, children }) => {
  ...
  return (
    <Link
      className={`w-32 flex items-center justify-center text-base whitespace-nowrap font-satoshi hover:font-bold pb-4 px-3 text-blue-500 ${
...
const Navbar: FC = () => {
  ...
  return (
    <header className="bg-[#07081a] grid grid-cols-2 md:grid-cols-5 justify-between items-center p-5">
      <nav>
        <div className="flex flex-col items-center justify-between w-full">
          <div className="text-lg font-semibold bg-[#07081a] w-full h-full py-2 text-blue-500 text-bold text-center">
            <Link
              className="flex items-center gap-4 cursor-pointer justify-center"
              href="/"
            >
              <ChainbaseIcon /> <span>ChainBase Hub 🔒</span>
            </Link>
          </div>
